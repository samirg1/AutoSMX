...
class SettingsPage(Page):
    def setup(self) -> None:
        ...
        ctk.CTkButton(self.frame, text="Manage Scripts >", command=lambda: self.change_page("SCRIPTS")).grid(column=0, row=3, sticky=ctk.W)
        ctk.CTkButton(self.frame, text="Search Parts >", command=lambda: SearchPartPopup(self.frame, self.storage.previous_parts)).grid(column=0, row=4, sticky=ctk.W)
