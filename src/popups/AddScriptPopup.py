...
from utils.constants import ON, OFF
...
class AddScriptPopup(Popup):
    def __init__(self, master: tkinter.Misc | None, storage: Storage):
        ...
        ctk.CTkLabel(self.frame, text="Script Basics").grid(column=0, row=0, columnspan=8, sticky=ctk.EW)
        number = ctk.StringVar()
        number_label = ctk.CTkLabel(self.frame, text="Number")
        ...
        tester_label = ctk.CTkLabel(self.frame, text="Tester Number")
        ...
        ctk.CTkEntry(self.frame, textvariable=tester_number).grid(column=3, row=1)
        nickname = ctk.StringVar()
        nickname_label = ctk.CTkLabel(self.frame, text="Nickname")
        nickname_label.grid(column=4, row=1)
        Tooltip(nickname_label, "Your own nickname for the script")
        nickname_entry = ctk.CTkEntry(self.frame, textvariable=nickname)
        nickname_entry.grid(column=5, row=1)
        go = ctk.CTkButton(self.frame, text="Go", command=lambda: self._get_script(number.get(), tester_number.get(), nickname.get()))
    ...
    def _get_script(self, number: str, tester_number: str, nickname: str) -> None:
        ...
        default_label = ctk.CTkLabel(self.frame, text="Default values")
        ...
        condition_label = ctk.CTkLabel(self.frame, text="Condition Line?")
        ...
        required_label = ctk.CTkLabel(self.frame, text="Required?")
        required_label.grid(row=2, column=4, sticky=ctk.W)
        Tooltip(required_label, "Mark this field as required")
        persistent_label = ctk.CTkLabel(self.frame, text="Persistent?")
        ...
        for i, line in enumerate(self.script.lines):
            ...
            if len(line.options) <= 1:
                ctk.CTkEntry(self.frame, textvariable=self.answers[i]).grid(column=2, row=row, columnspan=1, sticky=ctk.W)
            else:
                ctk.CTkSegmentedButton(self.frame, values=list(line.options), variable=self.answers[i]).grid(column=2, row=row, columnspan=1)

            if len(line.options) <= 1:
                ctk.CTkCheckBox(self.frame, text="", variable=self.conditions[i], onvalue=ON, offvalue=OFF, width=1, command=lambda i=i: self.set_required(i)).grid(column=3, row=row, sticky=ctk.EW)
                ctk.CTkCheckBox(self.frame, text="", variable=self.requireds[i], onvalue=ON, offvalue=OFF, width=1).grid(column=4, row=row, sticky=ctk.EW)
            ctk.CTkCheckBox(self.frame, text="", variable=self.non_persistents[i], onvalue=ON, offvalue=OFF, width=1).grid(column=5, row=row, sticky=ctk.EW)
            row += 1

        ctk.CTkButton(self.frame, text="Save Script", command=lambda: self.save_script(script_info)).grid(row=row, column=0, columnspan=8, sticky=ctk.EW)
    ...
    def save_script(self, script_info: ScriptInfo) -> None:
        ...
        for line, answer in zip(self.script.lines, self.answers):
            answer = answer.get()
